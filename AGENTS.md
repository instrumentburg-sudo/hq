...
12. [Утренняя сводка и недельный цикл](#12-утренняя-сводка-и-недельный-цикл)
...
| Проект | Путь | Стек | Приор. | Статус | Коммит |
|---|---|---|:-:|:-:|:-:|
| [personal-corp](#personal-corp) | `/home/iamsohappy/projects/Githab/personal corp` | HTML, JSON, транскрипты | P2 | active | 2026-05-12 |
| [crm](#crm) | `/home/iamsohappy/projects/Githab/crm` | Markdown карточки + frontmatter | P2 | active | 2026-05-12 |
...
**Контекст.** Уроки курса + извлечённые материалы. Урок 01 — HQ и агент-оркестрация; урок 02 — масштабирование без оверинженеринга (слои стратегии/исполнения, скиллы, Human Gate, Claude+Codex); урок 03 — от штаба к рабочей среде (полный транскрипт от 2026-05-12: три уровня парсинга данных в контекст, CRM-папка рядом со штабом, Codex Automation = cron в портативном файле, Meeting Copilot для живых созвонов, delta-фреймворк); лайв-Q&A потока 1 — промежуточный разбор между уроками 2 и 3 (страница `live-qa-1.html`, помечена «лайв-Q&A», не «урок N»). Публичные копии выжимок в `hq/course/` (`lesson{1,2,3}.html` + `live-qa-1.html`).
...
- **Путь:** `/home/iamsohappy/projects/Githab/crm` *(рядом со штабом, отдельный проект)*
- **Статус:** active · **Приоритет:** P2
- **Цель:** CRM-слой из урока 3 Personal Corp — карточки людей / компаний / сделок / встреч.
- **Стек:** Markdown карточки + frontmatter

**Контекст.** Скаффолд по канон-шаблонам Серёжи (`_templates/`): `10_Accounts/`, `20_People/{slug}/` (встречи внутри в `meetings/`), `30_Opportunities/`, `meeting-copilot-prompt.md`. Обязательное правило источника, обязательные поля контакта (telegram / last_touch / next_step / triggers-trust / weakens-position), стадии сделки, делта-лог, надзиратель по расписанию — в `crm/AGENTS.md`. Скаффолд 2026-05-12 (ДЗ урока 3), наполнение — за пользователем (первый реальный контакт + встреча, Meeting Copilot на одном живом созвоне).
...
Dashboard ориентирован **на текущий операционный фокус**, а не на полный реестр проектов. Текущая структура:

- **Направления** — сервис, площадки, тендера, личное: счётчики проектов/задач и главный фокус направления.
- **Текущие задачи** — короткий список приоритетов из `tasks/long/` и `tasks/short/`: статус, направление, проект, приоритет.
- **Применённые практики** — что из курса Personal Corp уже приземлено в работу.

В футере — 4 ссылки: полный реестр проектов (§6), оркестра OpenClaw (§11), курс Personal Corp — последний урок (`course/lesson{N}.html`, сейчас `lesson3.html`), репозиторий на GitHub.

Всё остальное (реестр 30 проектов, подробные карточки, подробности OpenClaw, прогресс курса) — **в AGENTS.md**, не на дашборде. Dashboard — операционная панель «что делать прямо сейчас», а не витрина проектов.
...
OpenClaw — рабочий runtime на VPS: он принимает живой контекст из Telegram, вызывает нужных исполнителей, работает с инструментами и возвращает связный результат. `hq/` остаётся стратегическим слоем: карта проектов, задачи, правила и dashboard.

Актуальное решение от 2026-04-25:

```text
HQ / GitHub Projects      = карта, задачи, источник правды
OpenClaw                  = исполнение и оркестрация
Telegram-топики           = пульт управления и живой контекст
topic-system              = главный архитектор / координатор
специализированные агенты = роли и способности, не обязательно видимые боты
```

Главное изменение: **агент ≠ Telegram-бот**. Видимый бот — только интерфейс, если он действительно нужен. По умолчанию агент считается внутренним исполнителем, которого вызывает `topic-system` или профильный topic-агент.

### 11.1 Слои системы

| Слой | Назначение | Источник правды |
|---|---|---|
| `hq/` | карта направлений, проекты, задачи, applied practices | `AGENTS.md`, `tasks/*`, `practices/*` |
| GitHub Projects / task-файлы | долговременный реестр задач и статусов | project items + `tasks/` |
| Telegram-топики | удобный интерфейс для Антона и текущего контекста | история топика |
| `topic-system` | главный архитектор: разбирает междоменные задачи, делегирует, собирает итог | системный топик + `workspace-topic-system` |
| topic-агенты | контекстные владельцы направлений | соответствующий Telegram-топик |
| специализированные агенты | исполнители с инструментами и доступами | их workspace/skills/tools |

### 11.2 Топики как рабочие комнаты

Топик — это **контекст работы**, а не отдельный бот. Если задача междоменная, архитектурная или требует нескольких исполнителей — её ведёт `topic-system`.

| Направление | Агент | Зона ответственности |
|---|---|---|
| Бизнес | `topic-business` | общие решения, операционный фокус, деньги/процессы без личных деталей |
| Стратегия | `topic-strategy` | стратегия, исследования, новые направления, архитектура бизнеса |
| Здоровье | `topic-health` | личный контур здоровья; действовать осторожно, с Human Gate |
| КП | `topic-kp` | коммерческие предложения, документы, шаблоны и сопровождение КП |
| Тендеры | `topic-tenders` | закупки, тендерные документы, подрядные контуры |
| Маркетплейсы | `topic-marketplaces` | Ozon/WB, карточки, остатки, отгрузки |
| Маркетинг Екб | `topic-marketing-ekb` | маркетинг ИнструментБург Екатеринбург |
| Маркетинг Тагил | `topic-marketing-tagil` | маркетинг ТагилИнструмент / prokatich |
| Система | `topic-system` | OpenClaw, агенты, правила, интеграции, диагностика, координация |

### 11.3 Реестр ролей

| Роль | Тип | Зона ответственности | Режим |
|---|---|---|---|
| `topic-system` | архитектор | общий контроль, делегирование, системные решения | главный координатор |
| `topic-business` | topic-агент | бизнес-контекст и операционные решения | рабочая комната |
| `topic-strategy` | topic-агент | стратегия и исследования | рабочая комната |
| `topic-health` | topic-агент | здоровье и личный контур | Human Gate |
| `topic-kp` | topic-агент | КП и документы продаж | рабочая комната |
| `topic-tenders` | topic-агент | тендеры и закупки | рабочая комната |
| `topic-marketplaces` | topic-агент | Ozon/WB и карточки | рабочая комната |
| `topic-marketing-ekb` | topic-агент | маркетинг Екатеринбург | рабочая комната |
| `topic-marketing-tagil` | topic-агент | маркетинг Нижний Тагил | рабочая комната |
| `dispatcher` | исполнитель / внешний интерфейс | маршрутизация задач сотрудникам | оставить видимым |
| `master` | исполнитель / внешний интерфейс | мастера, категории, выездные отчёты | оставить видимым |
| `sitebuilder` | исполнитель | сайт, SEO, деплой, frontend/backend сайта | внутрь системы |
| `marketer` | исполнитель | Метрика, Wordstat, Direct, аналитика маркетинга | внутрь системы |
| `advertiser` | исполнитель | Авито, Директ, объявления, креативы | внутрь системы |
| `chaser` | исполнитель | дожим, продажи, Ozon/WB/заказы | внутрь системы |
| `mailer` | исполнитель | почта, письма, входящие, рассылки | внутрь системы |
| `nutritionist` | личный сервис | питание и личные рекомендации | отдельный личный контур |

Легенда режимов:
- **главный координатор** — собирает задачу, выбирает исполнителей, возвращает один итог;
- **рабочая комната** — агент закреплён за Telegram-топиком и держит контекст направления;
- **внутрь системы** — старый Telegram-бот не нужен как видимая сущность, роль остаётся как агент/инструмент;
- **оставить видимым** — есть отдельная аудитория, права или внешний рабочий процесс;
- **Human Gate** — без автономных внешних действий и публикаций, только после подтверждения Антона.

### 11.4 Решение по Telegram-ботам

Не создавать новые видимые Telegram-боты без отдельной причины. Видимый бот оправдан, если есть хотя бы одно условие:

- с ним работают сотрудники, клиенты или подрядчики;
- нужны отдельные права, токен, лимиты или рискованный контур;
- нужен отдельный публичный интерфейс;
- нужна изоляция доступа: почта, сайт, реклама, финансы, здоровье.

Текущий миграционный статус:

| Группа | Что делать |
|---|---|
| Главный вход OpenClaw | оставить как основной транспорт в Telegram |
| `master` | оставить видимым: мастера и отчёты |
| `dispatcher` | оставить видимым временно: задачи сотрудников |
| `nutritionist` | оставить отдельным личным сервисом |
| `marketer`, `advertiser`, `sitebuilder`, `chaser`, `mailer` | постепенно убирать из видимого Telegram-слоя; оставить как внутренних исполнителей с workspace/skills/tools |

Важно: перенос роли внутрь **не удаляет** её workspace, skills, tools и память. Меняется только видимость и способ вызова.

### 11.5 Правила оркестрации

1. **Один видимый ответ лучше нескольких голосов.** Специалисты не должны соревноваться в Telegram. Координатор собирает результат и возвращает связно.
2. **Сложные задачи идут через `topic-system`.** Особенно если нужно несколько областей, диагностика системы, архитектурное решение или миграция.
3. **Профильные задачи остаются в своём топике.** КП — в КП, тендеры — в тендерах, маркетинг — в маркетинговых топиках.
4. **Долгие задачи фиксируются в HQ / GitHub Projects.** Telegram — живой контекст, но не главный реестр задач.
5. **Делегирование через OpenClaw:** `sessions_spawn`, `sessions_send`, субагенты и внутренние task-файлы. `context: "fork"` использовать только когда дочернему агенту реально нужен текущий разговор.
6. **Публичный dashboard не раскрывает приватное.** Не публиковать Telegram @handle, chat/topic IDs, токены, абсолютные пути, финансовые детали и личные данные.
7. **Не отключать старые боты массово.** Сначала инвентаризация, потом решение по каждому интерфейсу, потом конфиг с бэкапом и проверкой.

### 11.6 Когда создавать GitHub Project item

Создавать задачу в GitHub Projects / `tasks/`, если:

- работа дольше одного подхода;
- есть внешний исполнитель или несколько агентов;
- нужно помнить статус через дни/недели;
- есть риск потерять решение в Telegram-истории;
- результат влияет на сайт, продажи, деньги, сотрудников или процессы.

Не создавать item для разовых быстрых ответов, черновых вопросов и мелкой диагностики без продолжения.

---

## 12. Утренняя сводка и недельный цикл

Утренняя сводка — не список файлов из `tasks/`, а короткая управленческая картина на день. Она собирает четыре источника:

1. **GitHub Project «Антон — Всё»** — главная очередь: что в работе, что на неделе, что висит в очереди.
2. **Локальные задачи штаба** — правила, практики и задачи, которые живут в `hq/tasks/`.
3. **CRM рядом со штабом** — люди, встречи и сделки из `../crm/`, особенно просроченные следующие шаги.
4. **Недельный контур** — последний разбор недели, план недели и свежая приоритизация.

Скрипт контекста для утреннего сообщения:

```text
scripts/hq-task-summary-context.py — источник в репозитории
/home/iamsohappy/.hermes/scripts/hq-task-summary-context.py — ссылка для Hermes cron
```

Hermes-задача по расписанию:

```text
daily-hq-task-summary-ekb-1000 — ежедневно в 10:00 Екб, Telegram Антону
```

### 12.1 Правила сводки

- Сначала писать, что требует внимания сегодня.
- Если `hq/tasks` пустой, но в GitHub Project есть работа — прямо указывать расхождение, а не писать «задач нет».
- Очередь не считать задачей дня без решения Антона.
- CRM не имитировать: если карточек нет, писать «карточек нет» и следующий минимальный шаг.
- Telegram-историю не читать автоматически без белого списка чатов и отдельного разрешения.
- Писать по-русски, без лишних англицизмов, в предпринимательском контексте.

### 12.2 Недельный разбор
...
Файл недели: `WNN.md`, например `W20.md`.

Смысл: зафиксировать факт недели — что закрыто, что зависло, что переносится, что надо решить Антону, что агент может сделать сам.
...
Файл недели: `WNN-outcomes.md`, например `W20-outcomes.md`.

Смысл: выбрать 3–7 проверяемых результатов недели. Не список желаний, а то, что можно проверить к концу недели.

### 12.4 Human Gate

Агент может сам собрать контекст, черновик разбора и черновик плана. Создавать, переносить, закрывать или массово размечать задачи в GitHub Project — только после подтверждения Антона.
